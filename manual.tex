...
When moving the mouse over the page, you will notice that certain areas are highlighted in pink. These are the blocks of text that you can tag. Sometimes the pink areas are fairly small (single words or lines of text), sometimes they are pretty large (whole paragraphs, or even whole pages). So it makes sense to move the mouse around a little before you actually start tagging because sometimes you will want to mark big areas as, say, 'bad', and it saves you a lot of time if you do not have to tag every single line or paragraph. As a rule of thumb, it often makes sense to mark \emph{everything} in red ('bad'), from top to bottom, and only then to start tagging smaller pieces in yellow or green ('don't know' or 'good', respectively) (see also \textbf{\hyperref{Examples}{Examples, described on page~}{}{Examples}, \hyperref{Tips \& Tricks}{Tips \& Tricks, page ~}{}{TnT}}).\\ \\
...
For tagging a pink-highlighted section as 'good', 'bad', or 'don't know', you have two options: You can use (1) keyboard shortcuts (hotkeys) or you can use (2) the context menu (rightclick).\\
\begin{enumerate}
	\item Keyboard Shortcuts
		\begin{itemize}
			\item \emph{bad}: ctrl+alt+1
			\item \emph{don't know}: ctrl+alt+2
...
\htmladdimg{../images/krdwrd_menu.jpg}
...
\htmladdimg{../images/krdwrd_menu2.jpg} \\
The options in this menu make your life easier when tagging pages. \\
	\begin{itemize}
		\item {\large \textbf{Propagate}}\\
Here you can explicitly propagate a given tag down to all sibling nodes. This is helpful when you have a large portion that should be tagged red but all its siblings should be tagged green. You can then tag the parent node green, propagate, and re-tag the parent node as red. This way you don't need to tag all the siblings separately. (Try this on the \hyperref{Examples}{Examples, described on page~}{}{Examples}).
...
This menu option will send you to your krdwrd.org account. There you can see how many pages you have already tagged, and you can view and re-submit your tagged pages.
\end {itemize}
...
\label{Annotation}
In the previous section, we described how to use the tool and how to assign tags. In the following, we give you guidelines regarding \emph{which} the tags should be assigned to a particular kind of text.
...
\begin{itemize}
	...
	\end{enumerate}
	$\rightarrow$ Generally speaking, boilerplate is everything that can be used interchangeably with any other Web page or what could be thrown away without changing the general information of the page.
%	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item text that makes us of 'normal' words.
		\item text that is written in English.
	\end{enumerate}
\end{itemize}

Simple, is not it? You will notice that on some pages you can only highlight very large areas, on others the choices are less restricted. If you tag an element, the tag assigned is propagated to all elements that are contained in this area. However, if you are not sure whether a specific element is entailed, just tag it too to be on the safe side (remember the \emph{sidebar option} mentioned in the previous section!).\\

In a previous section, we said that as a rule, it often makes sense to mark \emph{everything} in red ('bad'), from top to bottom, and only then to start tagging smaller pieces in yellow or green ('don't know' or 'good', respectively). The easiest way to tag a whole page red is to mark the outermost rim of the page and tag that as 'bad'. Due to the tag propagation, the whole page is now tagged as 'bad'. If you want to make sure that this is so, check the sidebar (see above).\\

Due to the structure of some pages, you will have to make a judgment call: Overall, is there more 'good' text than 'bad' text? If so, then mark the whole area in green, if not, then tag it as 'bad'. \\
...
Depending on the size of your keyboard and your hands, this may really hurt after some pages. But you can change the shortcuts. All you need is the \htmladdnormallink{keyconfig}{http://forums.mozillazine.org/viewtopic.php?t=72994} addon.

\begin{itemize}
	\item You need to switch the proxy off to install the addon (remember the \htmladdnormallink{krdwrd Off-Line Proxy}{https://krdwrd.org/trac/wiki/WebProxy}?). To turn the proxy off, go to \emph{Preferences $ \rightarrow $ Advanced $ \rightarrow $ Network $ \rightarrow $ Settings...} and check \emph{No Proxy}.
	\item Once the proxy is turned off, \htmladdnormallink{install keyconfig}{http://mozilla.dorando.at/keyconfig.xpi}.
	\item don't forget to turn the proxy back on.
	...
	\item You might want to turn off automatic updates for addons, the proxy will block the update atempts anyway.
\end{itemize}
